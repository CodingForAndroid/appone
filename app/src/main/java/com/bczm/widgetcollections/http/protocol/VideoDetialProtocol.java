...
import com.bczm.widgetcollections.bean.GuessFavouriateBean;
 ...
 */
public class VideoDetialProtocol  {

    //获取 猜你喜欢
    public  List<GuessFavouriateBean>    getGuessFavouriate() {

        List<GuessFavouriateBean> list = null;
        try {
            list = new ArrayList<GuessFavouriateBean>();
            JSONObject jsonObject = new JSONObject(ConfigManage.GUESS_FAVOURIATE);
            JSONArray jsonArray = (JSONArray) jsonObject.opt("items");
            JsonHelper.JSONArrayToList(jsonArray, list, GuessFavouriateBean.class);
    ...
    //获取 当前播放视频url
    public VideoDecorationBean   getVideoCrrentPlay(){
